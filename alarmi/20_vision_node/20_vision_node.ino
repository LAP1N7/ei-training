...
// [+3] 사물 모델 교체. test_inferencing 은 band/usb 라서 미션 라벨(mouse/cup/phonecase)과
//   전혀 안 맞았다. team_project 는 cup/mouse/phonecase, 96x96 RGB(27648) 로 입력 형식이
//   test 와 같아서 camera.cpp 는 그대로 쓴다. 라이브러리를 갈아끼웠으니 첫 빌드는 --clean.
#include <team_project_inferencing.h>
#include <Wire.h>
...
// [+] 대시보드 상태머신에 따른 모드 전환.
//   idle/armed/ringing/mission 은 계속 발행하고, success/shutdown 에서만 조용히 한다.
//   /jpg 웹서버는 이 플래그와 무관하게 계속 돈다 (카메라 뷰 + 수집기가 쓴다).
//   모르는 type(예: 예전 timer)은 무시한다.
//
// [+2] 예전에는 "제스처 미션 중에는 비전 노드가 침묵" 이었다. 그러면 미션이 도는
...
void onCmd(char *topic, byte *payload, unsigned int len) {
  ...
  if (strstr(buf, "\"state\":\"mission\"")) {
    ...
    en = strstr(buf, "\"kind\":\"object\"") != NULL;   // 제스처 미션 중엔 침묵
#else
    en = true;                                        // 제스처 미션 중에도 계속 발행
#endif
...
void loop() {
  ...
  // [+] inferEnabled 가 꺼져 있으면 추론·발행을 건너뛴다 (/jpg 는 위에서 계속 산다).
