// day4 — SENSOR NODE 1 (gesture) — SoftAP 버전.
//
// 10_gesture_node 와 센서/추론 로직은 동일하고, Wi-Fi 만 STA 대신 SoftAP 로 띄운다.
// 즉 이 보드가 자체 Wi-Fi 망을 만들고, 브로커(노트북)와 다른 보드들이 여기에
// STA 로 접속해야 한다. 브로커 IP 는 이 AP 서브넷(기본 192.168.4.x) 안의 것으로
// 맞춰야 한다 (게이트웨이 192.168.4.1 은 이 보드 자신).
//
...
//
// 빌드:
//   arduino-cli compile --fqbn esp32:esp32:XIAO_ESP32S3:PSRAM=opi 11_gesture_node_ap
//   arduino-cli upload -p COM6 --fqbn esp32:esp32:XIAO_ESP32S3:PSRAM=opi 11_gesture_node_ap
...
// ---- config ----
// SoftAP 모드: 이 보드가 자체 Wi-Fi 를 띄운다. 브로커(노트북)와 다른 보드들은
// 이 SSID 에 STA 로 접속해야 하고, 브로커 IP 는 이 AP 서브넷(192.168.4.x)의
// 것으로 바꿔줘야 한다 (기본 게이트웨이 192.168.4.1 은 이 보드 자신).
const char *WIFI_SSID  = "gesture-ap";
const char *WIFI_PASS  = "12345678";
const char *MQTT_HOST  = "192.168.4.2";   // AP 에 STA 로 붙는 노트북(브로커)의 IP. 실제 접속 후 확인해서 맞출 것
...
void setup() {
  ...
  delay(500);
  Serial.println("\n==== day4 GESTURE NODE (AP) ====");

  initSensors();

  WiFi.mode(WIFI_AP);
  WiFi.softAP(WIFI_SSID, WIFI_PASS);
  Serial.print("[WIFI-AP] ssid="); Serial.print(WIFI_SSID);
